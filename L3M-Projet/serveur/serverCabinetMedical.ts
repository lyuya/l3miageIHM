...
function distributeIdInf(doc: XMLDocument): string {
    const L: Element[] = Array.from(doc.getElementsByTagName("infirmier") );
    const ids: string[] = L.map(ele => ele.getAttribute("id"));
    let idNum;
...
function init(port, applicationServerIP, applicationServerPort) {
    ...
    app.post( "/deleteInfirmier", (req, res) => {
            ...
            const infsEles: Element[] = Array.prototype.slice.call(infsNodeList);
